...
- `ZHIPU_CODING_PLAN_API_KEY`: Zhipu Coding Plan API key (shared by OpenAI- and Anthropic-compatible endpoints)
- `ZHIPU_CODING_PLAN_BASE_URL`, `ZHIPU_ANTHROPIC_CODING_PLAN_BASE_URL`: Optional overrides for Coding Plan base URLs (defaults: `https://api.z.ai/api/coding/paas/v4` and `https://api.z.ai/api/anthropic`)
